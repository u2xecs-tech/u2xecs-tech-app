...
class Evaluations extends React.Component {
    ...
    componentDidMount() {
        Auth.currentAuthenticatedUser().then((user) => {
            ...
            API.graphql({query: listEvaluations}).then((apiData) => {
                this.setState({
                    evaluations: apiData.data.listEvaluations.items.filter((e) => {
    ...
    filteredEvals = () => {
        ...
        return this.state.evaluations.filter((e) => {
            return e.status + 1 === this.state.filter
        });
    }
                        ...
                        <Container maxWidth={false}>
                            <Grid container spacing={3}>
                                <Grid item lg={12} sm={12} xl={12} xs={12}>
                                    <EvaluationsToolbar sx={{pb: 1}} filter={this.state.filter}
                                                        selectChange={this.selectChange} addEval={this.addEvaluation}/>
                                </Grid>
                                {this.filteredEvals().map((e) => (
                                    <Grid item lg={3} sm={6} xl={3} xs={12}>
                                        <Link to={`/evaluation/${e.id}`}>
                                            <EvaluationCard sx={{height: 140}} name={e.name} description={e.description}
                                                            status={e.status}/>
                                        </Link>
                                    </Grid>
                                ))}
